namespace BIMS.Web.Services
{
	public class ImageService : IImageService
	{
		...
		public async Task<(bool isUploaded, string? errorMessage)> UploadAsync(IFormFile image, string imageName, string folderPath, bool hasThumbnail)
		{
			...
			var imagesPath = $"{_webHostEnvironment.WebRootPath}{folderPath}";
			var thumbImagesPath = Path.Combine($"{_webHostEnvironment.WebRootPath}{folderPath}", "thumb");

			if (!Directory.Exists(imagesPath))
				Directory.CreateDirectory(imagesPath);

			if (!Directory.Exists(thumbImagesPath))
				Directory.CreateDirectory(thumbImagesPath);
